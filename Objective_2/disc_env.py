...
class DiffDriveEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # self._agent_location = np.array([1.5, -1, 0],dtype=np.float32)

        self._agent_location = np.array(self.np_random.uniform(low=-self.high*.8, high=self.high*.8,size=3),dtype=np.float32)
        ...
        self._target_location = np.array([-1,1.5],dtype=np.float32)
    ...
    def step(self, action):
        ...
        if distance_from_object < distance_from_object_old:
            ...
            reward +=-1
        reward = -.5*heading_error**2
        
        if distance_from_object < circle_size/pix_square_size*.9:  
            reward += 100
            terminated = True
            # print("completed")
