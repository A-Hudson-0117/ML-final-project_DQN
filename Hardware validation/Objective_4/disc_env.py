...
global pos_zb, pos_zc, pos_ob, pos_obstical
def currentPos1(msg):
        global pos_zb
...
def currentPos4(msg):
        global pos_obstical
        pos_obstical = msg.pose.position
...
rospy.Subscriber("/vrpn_client_node/ob/pose", PoseStamped, currentPos4)
...
class DiffDriveEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, robot_type=None, size=2.5):
        ...
        obsx = pos_obstical.x; obsy = pos_obstical.y

        self._obstical_location = np.array([obsx,obsy],dtype=np.float32)   
        self.xb_pos = pos_zb.x; self.yb_pos = pos_zb.y
        self.xc_pos = pos_zc.x; self.yc_pos = pos_zc.y

        heading = math.atan2(self.yb_pos -self.yc_pos,self.xb_pos -self.xc_pos)

        self._agent_location = np.array([self.xc_pos, self.yc_pos, heading],dtype=np.float32)
        ...
        self.observation_space = spaces.Dict({
                ...
                # "iterations": spaces.Box(0, 10000, shape=(1,), dtype=int),
    ...
    def _get_obs(self):
        ...
        return {"agent": self._agent_location, "target": self._target_location,
                ...
                "heading": self._objetive_heading} 
                # "iterations": self._time_elapsed}
    ...
    def step(self, action):
        ...
        if distance_from_object < distance_from_object_old:
            reward += 2 +1/(.2*distance_from_object) + 1/
        else:
            reward += -1
        
        
        # if distance_from_obstical < circle_size/pix_square_size*3:  
        #     reward += -2
        # else:
        #     reward+=2
        ...
        if self.robot_type == None and self._time_elapsed > 75:  
            terminated = True
            reward += -50
        elif self.robot_type == "real" and self._time_elapsed > 75:
            terminated = True
            reward += -50
